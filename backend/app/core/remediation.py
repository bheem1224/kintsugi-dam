        ...
        for file in quarantined_files:
            ...
            if file.last_hashed_date and file.last_hashed_date < threshold_date:
                try:
                    ...
                    logger.info(
                        f"Removed database record for pruned file: {file.filepath}"
                    )

                except FileNotFoundError:
                    # File is already gone, proceed with deleting the database record
                    logger.warning(
                        f"File {file.filepath} not found on disk, deleting DB record anyway."
                    )
                    await db_session.delete(file)
                    logger.info(
                        f"Removed database record for missing file: {file.filepath}"
                    )

                except (PermissionError, OSError) as e:
                    # Cannot delete due to permissions or other OS issues. DO NOT delete DB record.
                    logger.error(
                        f"Failed to remove physical file {file.filepath} due to OS error: {e}. Skipping DB record deletion."
                    )
